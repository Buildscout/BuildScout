  ...
  async function open(projectId,projectName){
    ...
    try{
      ...
      document.getElementById("intelBody").innerHTML=`
        ...
        <section class="intel-section"><div class="intel-title"><h3>Project team</h3><span>Only source-backed names are shown</span></div>${companies.length?`<div class="intel-list">${companies.map(x=>`<article class="intel-company"><div><small>${esc(x.role)}</small><b>${esc(x.company_name)}</b><p>${x.source?`Evidence: ${esc(x.source.source_name)}${x.source.source_record_id?` · ${esc(x.source.source_record_id)}`:""}`:"Source-backed project record"} · ${x.contact_count||0} verified contact${x.contact_count===1?"":"s"}</p>${x.website||x.phone?`<p>${esc([x.website,x.phone].filter(Boolean).join(" · "))}</p>`:""}</div>${badge(x.confidence)}</article>`).join("")}</div>`:`<div class="intel-empty">No verified project companies yet. BuildScout should enrich this project before recommending outreach.</div>`}</section>
        <section class="intel-section"><div class="intel-title"><h3>Decision makers</h3><span>Contact data remains tied to source evidence</span></div>${contacts.length?`<div class="intel-list">${contacts.map(x=>`<article><div><small>${esc(x.title||"Contact")}</small><b>${esc(x.full_name)}</b><p>${esc([x.email,x.phone].filter(Boolean).join(" · "))}</p></div>${badge(x.confidence)}</article>`).join("")}</div>`:`<div class="intel-empty">No verified decision-maker contacts yet. This company is eligible for source-backed contact research; BuildScout will not guess a person or email.</div>`}</section>
