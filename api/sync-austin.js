...
export default async function handler(req, res) {
  ...
  const maxRecords = Math.max(1, Math.min(Number(req.query?.limit || req.body?.limit || 1000), 5000));
  const pageSize = Math.max(25, Math.min(Number(req.query?.pageSize || req.body?.pageSize || 200), 1000));
