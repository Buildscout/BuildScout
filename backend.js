window.BuildScoutBackend = (() => {
  ...
  async function importProjects(projectList){if(!client)init();if(!Array.isArray(projectList)||projectList.length===0)return[];const rows=projectList.filter(p=>p&&p.permit_number).map(p=>({name:p.name||"Dallas Building Permit",city:p.city||"Dallas, TX",street_address:p.street_address||null,zip_code:p.zip_code||null,latitude:p.lat??null,longitude:p.lon??null,project_type:p.type||"Commercial",stage:p.stage||"Permit approved",estimated_value:Number(p.value||0),opportunity_score:Number(p.score||70),units:p.units??null,expected_start:null,general_contractor:p.contractor||null,permit_number:String(p.permit_number),source_name:p.source||"City of Dallas OpenData",source_url:"https://www.dallasopendata.com/Services/Building-Permits/e7gq-4sah",last_verified:new Date().toISOString().slice(0,10)}));const savedRows=[];for(const row of rows){const{data:existing,error:lookupError}=await client.from("projects").select("id").eq("source_name","City of Dallas OpenData").eq("permit_number",row.permit_number).maybeSingle();if(lookupError)throw lookupError;if(existing){const{data,error}=await client.from("projects").update(row).eq("id",existing.id).select();if(error)throw error;if(data)savedRows.push(...data);}else{const{data,error}=await client.from("projects").insert(row).select();if(error)throw error;if(data)savedRows.push(...data);}}return savedRows;}
