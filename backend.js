window.BuildScoutBackend = (() => {
  ...
  async function getProjectDocumentCounts(){if(!client)init();const counts={};let from=0;const pageSize=1000;while(true){const{data,error}=await client.from("project_documents").select("project_id").eq("is_public",true).range(from,from+pageSize-1);if(error)throw error;(data||[]).forEach(row=>{const key=String(row.project_id);counts[key]=(counts[key]||0)+1;});if(!data||data.length<pageSize)break;from+=pageSize;}return counts;}
  ...
  return {configured,init,getClient,signUp,signIn,signOut,getSession,getProjects,getProjectDocuments,getProjectDocumentCounts,importProjects,getSavedProjects,saveProject,unsaveProject,getPipeline,updatePipeline,getContacts,saveContact,deleteContact,setPrimaryContact,getActivities,addActivity,completeActivity,getAlerts,saveAlert,updateAlert,deleteAlert,getMatchingProjects};
})();
