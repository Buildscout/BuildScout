(() => {
  ...
  let mounting=false,lastProjectId=null;
  function roleMatch(contacts,re){return (contacts||[]).some(c=>re.test(String(c.role||'')));}
  function projectIdFromModal(){const btn=document.querySelector('#crm-modal [onclick*="saveOpportunity"]');return (btn?.getAttribute('onclick')||'').match(/saveOpportunity\('([^']+)'\)/)?.[1]||null;}
  function build(project,pipeline,contacts,docs,activities){
    const value=Number(pipeline?.opportunity_value||project?.estimated_value||0),probability=Number(pipeline?.probability||0),hasGC=Boolean(project?.general_contractor),hasDecisionMaker=roleMatch(contacts,/estimat|project manager|\bpm\b|owner|developer/i),hasContact=(contacts||[]).some(c=>c.phone||c.email),hasPlans=(docs||[]).length>0,hasNext=Boolean(pipeline?.next_action||pipeline?.follow_up_at),touched=(activities||[]).length>0;
    let score=20;if(value>=100000)score+=8;if(value>=500000)score+=5;if(probability>=20)score+=8;if(probability>=50)score+=5;if(hasGC)score+=12;if(hasDecisionMaker)score+=15;if(hasContact)score+=8;if(hasPlans)score+=10;if(hasNext)score+=6;if(touched)score+=3;score=clamp(score);
    const blockers=[];if(!hasGC)blockers.push('General contractor is not identified');if(!hasDecisionMaker)blockers.push('Estimator / PM / owner is not identified');if(!hasContact)blockers.push('No direct phone or email is saved');if(!hasPlans)blockers.push('No plans or specifications are available');if(!hasNext)blockers.push('No next action or follow-up is scheduled');
  ...
  async function mount(projectId){
    ...
    try{
      ...
      const project=(projects||[]).find(p=>String(p.id)===String(projectId));if(!project)return;
      const pipeline=(pipes||[]).find(p=>String(p.project_id)===String(projectId))||{},r=build(project,pipeline,contacts,docs,activities);styles();
      body.querySelectorAll('[data-phase10]').forEach(el=>el.remove());
      const section=document.createElement('section');section.className='phase10';section.dataset.phase10='1';section.dataset.projectId=String(projectId);
      ...
      const metrics=body.querySelector('.crm-metrics');
      if(metrics)metrics.insertAdjacentElement('afterend',section);else body.prepend(section);
      lastProjectId=projectId;
    }catch(e){console.warn('Phase 10 mount skipped:',e);}finally{mounting=false;}
  }
  function check(){const modal=document.getElementById('crm-modal');if(!modal){lastProjectId=null;return;}const id=projectIdFromModal();if(id)mount(id);}
  setInterval(check,300);
  window.addEventListener('load',check);
  window.BuildScoutPhase10={mount,check};
})();
